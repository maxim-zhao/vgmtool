...
void Trim(char* filename, int start, int loop, int end, BOOL OverWrite, BOOL PromptToPlay)
{
	...
	if (PromptToPlay)
	{
		if (ShowQuestion(
			...
			ShellExecute(hWndMain, "Play", Outfilename,NULL,NULL,SW_NORMAL);
	}
...
void Optimize(char* filename)
{
	...
	if (ShowQuestion(
		...
		ShellExecute(hWndMain, "Play", filename,NULL,NULL,SW_NORMAL);
}
...
void LoadFile(char* filename)
{
	...
	if (CurrentFileVGMHeader.VGMIdent != VGMIDENT)
	{
		// no VGM marker
		ShowError(
	...
	if (GD3Strings)
	{
		...
		for (int i = 0; i < NumGD3Strings; ++i)
		{
			...
			case 10: // Notes: change \n to \r\n so Windows shows it properly
				{
					wchar* wp = ModifiedString + wcslen(ModifiedString);
...
void UpdateGD3()
{
	...
	char* Outfilename = malloc(strlen(Currentfilename) + 10);
...
void CopyLengthsToClipboard()
{
	...
	char* p = GlobalLock(CopyForClipboard);
...
void ConvertDroppedFiles(HDROP HDrop)
{
	...
	{
		// Get filename length
		int FilenameLength = DragQueryFile(HDrop, i,NULL, 0) + 1;
		char* DroppedFilename = malloc(FilenameLength); // Allocate memory for the filename
...
void MakeTabbedDialogue()
{
	...
	// Get display rect of the tab control
	...
	// Enable WinXP styles
	{
		...
		if (dllinst)
		{
			...
			if (
				...
				(IsThemeDialogTextureEnabled(hWndMain)))
			{
				// and app is themed
				for (i = 0; i < NumTabChildWnds; ++i) EnableThemeDialogTexture(TabChildWnds[i], 6);
	...
	for (i = 0; i < NumTabChildWnds; ++i)
		SetWindowPos(TabChildWnds[i],HWND_TOP, TabDisplayRect.left, TabDisplayRect.top,
...
{
	char* buf = malloc(strlen(items) + 1);
...
LRESULT CALLBACK DialogProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: // Initialise dialogue
		...
		auto hIcon = LoadIcon(HInst,MAKEINTRESOURCE(MAINICON));
