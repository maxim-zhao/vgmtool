...
void write_to_text(char* filename)
{
	...
	int i;
	// We write into these
	char* noiseTypes[5][64] = {{"high (", "med (", "low (", "ch 2"}};
	...
	do
	{
		...
		// Analyse:
			...
			case 4: // Tone registers
				{
					float Freq;
					if (PSGRegisters[PSGLatchedRegister]) Freq = vgmHeader.PSGClock / 32 / (float)PSGRegisters[
						PSGLatchedRegister];
					else Freq = 0.0;
					fprintf(out,
			...
			case 0x2: // Tone more stuff including key
				...
				else
				{
					if (b2 & 0x10)
					{
						...
						freq = (double)ym2413FNumbers[chan] * vgmHeader.YM2413Clock / 72 / (1 << 19 - ym2413Blocks[
							chan]);
			...
			case 0x3: // Tone instruments and volume/percussion volume
				...
				else
				{
					fprintf(out, "Tone vol/instrument: ch %1d -> vol 0x%1x = %3d%%; inst 0x%1x = %-17s", b1 & 0xf,
					        b2 & 0xf, (int)((15 - (b2 & 0xf)) / 15.0 * 100), b2 >> 4, ym2413Instruments[b2 >> 4]);
					if (b1 >= 0x36)
					{
						int i1, i2 = -1;
						switch (b1)
						...
						fprintf(out, " OR Percussion volume:   %s -> vol 0x%1x = %3d%%",
						        ym2413RhythmInstrumentNames[i1], b2 & 0xf, (int)((15 - (b2 & 0xf)) / 15.0 * 100));
						if (i2 > -1) fprintf(out, "; %s -> vol 0x%1x = %3d%%", ym2413RhythmInstrumentNames[i2], b2 >> 4,
						                     (int)((15 - (b2 >> 4)) / 15.0 * 100));
					}
					fprintf(out, "\n");
				}
				break;
			...
			case 0x2:
				...
				case 0x27: // Timer control/ch 3 mode
					...
					fprintf(out, "ch 3 %s\n",
					        (b2 & 0xc0) == 0
						        ? "normal mode"
						        : (b2 & 0xc0) == 1 ? "special mode" : "invalid mode bits");
					break;
